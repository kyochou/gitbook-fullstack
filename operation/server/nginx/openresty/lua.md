# Lua
在 OpenResty 中选择使用库时, 有一个基本原则: 尽量使用 OpenResty 的库函数, 尽量不用 Lua 的库函数, 因为 Lua 的库都是同步阻塞的.
...
* `#`: 取字符串或表的长度. `#str, #table`.
* `.`, `:`: 冒号操作会带入一个 `self` 参数, 用来代表自己. 而点号操作, 只是内容的展开. 在函数定义时, 使用冒号将默认接收一个 `self` 参数, 而使用 `.` 则需要显式传入 `self` 参数:
    ```lua
    local str = 'abcde'
    print(str:sub(1, 2), str.sub(str, 1, 2) -- 效果相同
